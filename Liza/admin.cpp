...
void Admin::change_order_status(const std::string &order_id,
    ...
    connectExec(R"(UPDATE "ORDER" set "STATUS" = ')" +
                         orderStatuses[set_status] +
                         "' where \"ORDER_ID\"=" + order_id);
}

void Admin::change_dish_status(const std::string &dish_id,
                                        int set_status) {
    connectExec(R"(UPDATE "DISH" set "STATUS" = ')" +
                         dishStatuses[set_status] +
...
std::string Admin::getPrice(const std::string &dish_id) {
    return connectGet(
        R"(SELECT "PRICE" FROM "DISH" WHERE "DISH_ID" = ')" + dish_id + "'");
}
...
std::string Admin::getDishName(const std::string &dish_id) {
    return connectGet(
        R"(SELECT "DISH_NAME" FROM "DISH" WHERE "DISH_ID" = ')" + dish_id + "'");
}
} // namespace restbes